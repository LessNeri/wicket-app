...
public class PermisoPage extends BasePage {
    ...
    public PermisoPage() {
        ...
        Label mensajeSoloLectura = new Label("mensajeSoloLectura",
                "Modo solo lectura. No tienes permisos para editar, contacta a un administrador.");
            ...
            protected void populateItem(ListItem<Modulo> parentItem) {
                ...
                parentItem.add(new ListView<Modulo>("moduloList", hijos) {
                    ...
                    protected void populateItem(ListItem<Modulo> item) {
                        ...
                        boolean esPerfilSeleccionadoAdmin = perfilSeleccionado != null
                                && perfilSeleccionado.isBitAdministrador();

                        // Los checks se habilitan SI el usuario logueado es admin Y el perfil
                        ...
                        // SI el perfil seleccionado es ADMIN, forzamos todos los permisos a TRUE
                        // irrevocablemente
                        ...
                        item.add(createCheckbox("agregar", modulo.getId(), "bitAgregar", checksHabilitados));
                        item.add(createCheckbox("editar", modulo.getId(), "bitEditar", checksHabilitados));
                        item.add(createCheckbox("eliminar", modulo.getId(), "bitEliminar", checksHabilitados));
                        item.add(createCheckbox("consultar", modulo.getId(), "bitConsulta", checksHabilitados));
    ...
    private void cargarPermisos() {
        if (perfilSeleccionado != null) {
            System.out.println("\n=== CARGANDO PERMISOS PARA PERFIL ===");
            System.out.println("Perfil ID: " + perfilSeleccionado.getId());
            System.out.println("Perfil Nombre: " + perfilSeleccionado.getStrNombrePerfil());
            ...
            for (PermisoPerfil p : permisos) {
                System.out.println("  - Módulo " + p.getIdModulo() +
                        " | C:" + p.isBitConsulta() +
                        " | A:" + p.isBitAgregar() +
                        " | E:" + p.isBitEditar() +
                        " | D:" + p.isBitEliminar());
    ...
    private void guardarPermisos() {
        ...
        for (PermisoPerfil p : listaParaGuardar) {
            p.setIdPerfil(perfilSeleccionado.getId());
            System.out.println("  Módulo " + p.getIdModulo() +
                    " | C:" + p.isBitConsulta() +
                    " A:" + p.isBitAgregar() +
                    " E:" + p.isBitEditar() +
    ...
    private CheckBox createCheckbox(String id, final int moduloId, String property, boolean enabled) {
        CheckBox check = new CheckBox(id, new PropertyModel<>(permisosMap.get(moduloId), property));
