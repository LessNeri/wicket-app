...
public UsuarioPage() {
        ...
        WebMarkupContainer paginacionContainer = new WebMarkupContainer("paginacionContainer");
        paginacionContainer.setOutputMarkupId(true);
        paginacionContainer.setVisible(puedeConsultar);
        add(paginacionContainer);
        ...
        busquedaField.add(new AjaxFormComponentUpdatingBehavior("keyup") {
            ...
            protected void onUpdate(AjaxRequestTarget target) {
                ...
                paginaActual = 1;
                cargarDatos();
                // AQUÍ ESTÁ LA MAGIA: Solo recargamos tabla y paginación
                target.add(tablaContainer, paginacionContainer);
            }
        });
        busquedaForm.add(busquedaField);
        add(busquedaForm);

        ListView<Usuario> listaUsuarios = new ListView<Usuario>("usuarioList", new org.apache.wicket.model.PropertyModel<>(this, "usuarios")) {
            @Override
            protected void populateItem(ListItem<Usuario> item) {
                ...
                String apellidoPaternoStr = usuario.getStrApellidoPaterno() != null ? usuario.getStrApellidoPaterno().trim() : "";

                String primerNombre = nombreStr.contains(" ") ? nombreStr.split(" ")[0] : nombreStr;
                String primerApellido = apellidoPaternoStr.contains(" ") ? apellidoPaternoStr.split(" ")[0] : apellidoPaternoStr;

                String imgUrl = usuario.getStrImagenUrl();
                
                if (imgUrl == null || imgUrl.trim().isEmpty()) {
                    String nombreParaAvatar = primerNombre + "+" + primerApellido;
                    imgUrl = "https://ui-avatars.com/api/?name=" + nombreParaAvatar + "&background=random&color=fff&size=128";}
                
                org.apache.wicket.markup.html.WebMarkupContainer imgContainer = new org.apache.wicket.markup.html.WebMarkupContainer("imagen");
                ...
                BookmarkablePageLink<Void> editarLink = new BookmarkablePageLink<>("editarUsuario", EditarUsuarioPage.class, params);
                editarLink.setVisible(puedeEditar);
                item.add(editarLink);
                
                BookmarkablePageLink<Void> eliminarLink = new BookmarkablePageLink<>("eliminarUsuario", EliminarUsuarioPage.class, params);
...
add(feedback);
        ...
        Label infoPag = new Label("infoPagina", new org.apache.wicket.model.PropertyModel<>(this, "textoPaginacion"));
        paginacionContainer.add(infoPag);

        AjaxLink<Void> btnAnterior = new AjaxLink<>("btnAnterior") {
            @Override
        ...
        paginacionContainer.add(btnAnterior);

        AjaxLink<Void> btnSiguiente = new AjaxLink<>("btnSiguiente") {
            @Override
            public void onClick(AjaxRequestTarget target) {
        ...
        paginacionContainer.add(btnSiguiente);
    }
